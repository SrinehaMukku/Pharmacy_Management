...
login=sqlite3.connect("security.db")
l=login.cursor()
...
def open_win(): #OPENS MAIN MENU----------------------------------------------------------------------------MAIN MENU
    ...
    Button(apt,text='New Client', width=25, command=val_cus).grid(row=4,column=0)
   ...
   ## Button(apt,text='Modify',width=15, command=modify).grid(row=4,column=1)
    Button(apt,text='Search', width=15, command=search).grid(row=4,column=1)
    Button(apt,text='Expiry Check', width=15, command=exp_date).grid(row=5,column=1)
    ...
    Label(apt, text='__________'*8).grid(row=12,column=0,columnspan=3)
...
def delete_stock(): #OPENS DELETE WINDOW--==--------------------------------------------------DELETES A PARTICULAR STOCK ITEM
    ...
    Label(d,text='',width=30,bg='red').grid(row=0,column=1)
    ...
    b=Button(d,width=20,text='Back',command=main_menu).grid(row=5,column=3)
    d.mainloop()
...
def modify():    # window for modification-----------------------------------------------------------------------MODIFY
    ...
    col=('', '', 'type', 'qty_left', 'cost', 'purpose', 'expdt', 'loc', 'generic_name')
    col_n=('', '', 'Type', 'Quantity Left', 'Cost', 'Purpose', 'Expiry Date', 'Rack location', 'generic_name')
    flag='st'
    name_=''
    apt.destroy()
    n=[]
    ...
    Button(st,width=10,text='Back',command=main_menu).grid(row=5,column=5)
    st.mainloop()
...
def stock():    #add to stock window------------------------------------------------------------------------ADD TO STOCK
    ...
    Label(sto,text='Exp           Rack   generic_name                      ').grid(row=14,column=5)
    Button(sto,width=10,text='Back',command=main_menu).grid(row=12,column=5)
    ref()
    sto.mainloop()
...
def ref(): # creates a multi-listbox manually to show the whole database
    ...
    def onmousewheel():
        lb1.ywiew=('scroll',eventL.delta,'units')
        lb2.ywiew=('scroll',event.delta,'units')
...
def exp_date(): # expiry window open-----------------------------------------------------------------------------EXPIRY
    ...
    if flags=='apt1':
        Button(exp,text='Back', command=main_cus).grid(row=5, column=2)
    else:
        Button(exp,width=20,text='Check Products expiring', command=exp_dt).grid(row=5, column=0)
        Button(exp,text='Back', command=main_menu).grid(row=5, column=2)
    exp.mainloop()
...
def s_exp():    # shows the expiry date of the medicine entered
    ...
    for i in cur:
        if(i[1]==s.get()):
            ...
            if d1>d2:
                Label(exp, text='EXPIRED! on '+i[6]).grid(row=6, column=0)
                top=Tk()
...
def billing(): # to create bills for customer-------------------------------------------------------------BILLING system
    ...
    Label(st,text='named ').grid(row=3,column=0)
    name1=Entry(st)
    name1.grid(row=3, column=1)
    #__________________________
    Label(st,text='ID').grid(row=1,column=0)
    vc_id=Entry(st)
    vc_id.grid(row=1, column=1)
    #____________ADDRESS
    Label(st,text="address ").grid(row=2, column=0)
    add=Entry(st)
    add.grid(row=2, column=1)
    #_____________________
    Button(st,text='Check in stock', command=blue).grid(row=4, column=0)
    ...
    refresh()
    Button(st,width=15,text='Back', command=main_menu).grid(row=9,column=6)
...
def refresh():
    ...
    for i in cur:
        ...
        lb2.insert(cx,' '+str(i[7])+'        '+str(i[3])+'             LE '+str(i[4]))
    c.commit()
...
def blue(): # check if valued customer
    ...
    for i in cur:
        if vc_id.get()!='' and int(vc_id.get())==i[2]:
            named=i[0]
            addd=i[1]
            Label(st,text=named,width=20).grid(row=3, column=1)
            Label(st,text=addd,width=20).grid(row=2, column=1)
            Label(st,text=i[2],width=20).grid(row=1, column=1)
...
def make_bill(): # makes bill
    ...
    if t==1:
        ...
        m+="-----------------------------------------------\n"
        m+='Total'+(' '*25)+(' '*(12-len(str(ntotal)))) +'LE '+ str(ntotal)+'\n'
        det[3]=str(ntotal)
    else:
        m+='Total'+(' '*25)+(' '*(12-len(str(total)))) +'LE '+ str(total)+'\n'
        det[3]=str(total)
...
def show_rev(): # opens revenue window-----------------------------------------------------------------------TOTAL REVENUE
    ...
    for i in cur:
        if i[4]==today:
            total+=float(i[3])
    print (total)
    Label(rev,width=22,text='Total revenue: LE '+str(total), bg='dimgray',fg='white').grid(row=1,column=0)
    cx=0
    ...
    for i in cur:
        if i[4]==today:
            cx+=1
            lb1.insert(cx,'Bill No.: '+str(i[5])+'    : LE '+str(i[3]))
    c.commit()
    Button(rev,text='Back',command=main_menu).grid(row=15,column=0)
    rev.mainloop()
...
def search():   #search window medicine and symptom details---------------------------------SEARCH MEDICINE RACK & SYMPTOMS
    ...
    if flags=='apt1':
        Button(st,width=15, text='Back', command=main_cus).grid(row=6, column=2)
    else:
        Button(st,width=15, text='Back', command=main_menu).grid(row=6, column=2)
    st.mainloop()
...
def search_med():
    ...
    for i in cur:
        if i[5]==sym.get():
            y.append(str(i[0])+ ' '+str(i[1])+'  price :'+ str(i[4])+' LE '+'    Rack : '+str(i[7])+'    generic_name:  '+str(i[8]))
            x=x+1
    top=Tk()
...
def val_cus():  #to enter new valued customer-----------------------------------------------------------NEW VALUED CUSTOMER
    ...
    flag='val'
    val=Tk()
    Label(val,text="Enter valid customer details ").grid(row=0,column=0,columnspan=3)
    Label(val,text="-"*60).grid(row=1,column=0,columnspan=3)
    Label(val,text="Name").grid(row=3,column=0)
    name_vc=Entry(val)
    name_vc.grid(row=3  , column=1)
    Label(val,text="Address").grid(row=4,column=0)
    add_vc=Entry(val)
    add_vc.grid(row=4, column=1)
    Label(val,text="Id").grid(row=2,column=0)
    vc_id=Entry(val)
    vc_id.grid(row=2, column=1)
    Button(val,text='Submit',command=val_get).grid(row=5, column=1)
    Button(val,text='Back',command=main_menu).grid(row=5, column=2)
...
def again():    #for login window-----------------------------------------------------------------------------LOGIN WINDOW
    ...
    root=Tk()
    root.title('pharmacy')
  #  label0 = Label(root, text="PHARMACY MANAGEMENT SYSTEM ", bg="grey", fg="white", font=("Cambria Bold", 40),
              #     border='2')
    Label(root,text='Life Pharmacy',font=("Cambria Bold", 20),
                   border='2').grid(row=0,column=0,columnspan=5)
  #  Label(root,text="pharmacy").grid(row=1,column=0,columnspan=5)
    Label(root,text='_______________________________________________________',).grid(row=2,column=0,columnspan=5)
    ...
    Button(root,width=6,text='Login',command=check,bg='lightgray').grid(row=3, column=3)
    Button(root,width=6,text='Close',command=root.destroy,bg='lightgray').grid(row=4, column=3)
    root.mainloop()
